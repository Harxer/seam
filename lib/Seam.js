...
let stunUrl = "";
export function setStunUrl(url) {
    stunUrl = url;
}
...
export const getOpenPeers = () => getPeers().filter(peer => peer.chatData() || peer.isChatOpen()) // TODO include all streams?
...
class Connection {
    constructor(guid, statusCallback = () => {}) {
        ...
        // Since RelayServer has a policy that aims to never re-assign the same ID number
        // to the same clent, when a client disconnects from Relay, we mark any connections
        // as "stale" to inform peers that further links cannot be made on that Conenction.
        this.isStale = false

        /// WebRTC object with ICE callback suite setup
        if (stunUrl === undefined || stunUrl.length === 0) throw new Error('Stun URL not set.')
        this.rtc = RTCHelper.initRtc(this, stunUrl)
    ...
    public() {
        return {
            ...
            guid: () => this.guid,
            stale: () => !isRelayConnected() || this.isStale,
            reject: () => this.reject()
        }
    }
    ...
    handleAnswer(rtcDescriptor, hxDescriptor) {
        if (!RTCHelper.eqSet(this.acceptedChannels, hxDescriptor)) return
        this.rtc.setRemoteDescription(new RTCSessionDescription(rtcDescriptor))
    }

    /** Add ICE candidates from received candidate packets. */
    handleCandidate(rtcDescriptor, hxDescriptor) {
        if (!RTCHelper.eqSet(this.acceptedChannels, hxDescriptor)) return
    ...
    /** Handle peer rejecting connection - cleanup requests. */
    handleReject(_, hxDescriptor) {
        if (hxDescriptor.size === 0) {
          delete requests[this.guid]
        } else if (hasRequest(this.guid)) {
          requests[this.guid] = hxDescriptor
        }
        notifyRequest(this.guid)
    }
    ...
    send(rtcDescriptor, typeOverride = undefined) {
        let type = typeOverride || rtcDescriptor.type
        let data = {hxDescriptor: Array.from(this.acceptedChannels), rtcDescriptor, type}
    ...
    /** close() happens on both sides, reject happens from the peer that instigated close() */
    reject() {
      this.clearAcceptedChannels()
      this.send(this.rtc.localDescription, 'reject')
      this.close()
    }
}
...
export function handleRelay(message) {
    ...
    const relayHandlers = {
        ...
        'answer': (a, b) => connection.handleAnswer(a, b),
        'candidate': (a, b) => connection.handleCandidate(a, b),
        'closed': () => connection.handleClosed(),
        'reject': (a, b) => connection.handleReject(a, b)
    }

    let handler = relayHandlers[message.data.type]
    if (typeof handler !== 'function') return
    handler(message.data.rtcDescriptor, new Set(message.data.hxDescriptor))
}

/**
 * Handle Relay server exit messages. Should come from a RelayClient.
 * @param {GUID} source - Exit packet from Relay.
 */
export function handleExit(source) {
  if (!hasConnection(source)) return // If connection was not setup, ignore

  let connection = getConnection(source)
